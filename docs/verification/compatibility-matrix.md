...
| --- | --- | --- | --- | --- | --- |
| Sodium | [`mc26.2-0.9.1-fabric`](https://modrinth.com/mod/AANobbMI/version/2Yom1N68) | real macOS remote client, VFX-009 normal/reduced/fallback matrix | SUPPORTED | Full implementation/evidence commit `e78bf8f01fc79d6e05838c083685d348c214502c` loaded the pinned runtime and completed nine digest-bound upward-camera rows: normal and reduced motion at distances 4/12/24, rain-command, resource-reload, and an explicitly disabled `STATIC_WHITE` fallback. All nine original-resolution frames passed positive silhouette/tonal/fallback criteria. | Normal visibly shows the large warm-white/gold radial rune/halo; reduced motion has two large static lower-contrast silhouettes; fallback has no enhanced silhouette. The immutable-cache Sodium JAR is not stored in Git; name/version/size/SHA/source URL and loaded-mod/log/config/options receipts are retained. The rain command succeeded but the no-skylight client observed `weather=clear`, so no rendered-rain claim is made. No third-party resource-pack live matrix was captured; source/architecture checks are not visual proof. |
