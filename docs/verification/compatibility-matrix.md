...
| --- | --- | --- | --- | --- | --- |
| Sodium | [`mc26.2-0.9.1-fabric`](https://modrinth.com/mod/AANobbMI/version/2Yom1N68) | real macOS remote client, normal + reduced motion | SUPPORTED | Apple M3 Pro Sodium renderer starts; resources/models/atlases bake; visually inspected clean frames prove the unpaused static-white Light Realm, ten-symbol HUD/power rail, authored Rank Maze screen, and current Double Health semantic pulse. With particles minimal/effect scale zero, the same surfaces render and the pulse has visibly fewer particles. | This proves the current static renderer and accessibility behavior. It does **not** claim the future enhanced VFX-009 renderer. |
...
Use Java 25 and run profiles sequentially. `runCompatibilityGameTest` hash-verifies and stages the complete server profile into `build/compatibility-runs/complete-gametest`; ordinary `runGameTest` clears its own `mods/` and remains unpolluted. The harness rejects unsafe IDs/types/sizes/hashes/URLs/profiles, Modrinth URL-ID mismatches, missing/symlinked artifacts, and arbitrary/equal/symlink-escaping run directories. It writes an exact receipt to each isolated game directory.

Every claimed client log is the exact `<gameDir>/logs/latest.log` for the named process and receipt. The normal and reduced clients used distinct run directories, role markers, timestamps, and screenshots. Complete logs were passed through the deterministic sanitizer and committed rather than referenced from ignored mutable paths.
