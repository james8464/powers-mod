...
The exact client command has a fresh full wrapper transcript and machine receipt recording `BUILD SUCCESSFUL`, exit 0, 971 unique captures, 9,034 unique capture IDs, and 971 verified raw digests. The final-review scale regression tests and focused JVM/resource gates passed. The gallery server test passed 1/1. The accepted authoritative `./test.sh gametest` execution printed `All 131 required tests passed` and `BUILD SUCCESSFUL`; its surrounding tee wrapper later hit zsh's reserved `status` variable, so that wrapper is truthfully recorded as nonzero outside Gradle. Four literal final-review `check` attempts are retained: the first, second, and fourth failed in different aggregate-only GameTests; the third passed all 131 GameTests and then failed only because the in-progress successor checksums and receipt-SHA test still named the previous evidence state. Those two evidence-contract failures were corrected and passed 21/21 independently, but no post-finalization literal `check` obtained a clean single-command exit.
