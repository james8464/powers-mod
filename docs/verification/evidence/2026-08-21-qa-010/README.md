...
| Real bounded world border | Time Shift exact-coordinate input with mounted player/horse/zombie, nearby Shadow, and active Astral body session | rejected without nearest-safe substitution; every location, exact riding edge, and body/session identity stayed unchanged | 0 energy; 0 coordinate rewrites; 0 owner ticket | body/session/entities removed; prior center and size restored | `powers-gametest:hostile_environment_game_tests_world_border_rejects_exact_coordinates_atomically_without_rewriting` |
...
- Near-cap stable-order fairness RED: consuming one anchor advanced by 64 and skipped 63 unvisited anchors. GREEN: the O(64) integer cursor advances by actual visited count; unit coverage proves 65/100 stable-list round robin, early-break resumption, reset, and bounded/index-valid traversal over actual UUID lists after representative remove/add/reorder churn. It does not claim identity-fair resumption under arbitrary reordering: an explicit UUID RED proved the numeric cursor can repeat a moved identity, while avoiding that would require an O(all) lookup or a separately maintained join/leave index.
...
- Fixture-lifecycle review: mounted, void, and synthetic async fixtures install idempotent cleanup guards immediately, run them before timeout, and invoke them from every synchronous and delayed assertion failure/final path.

## Verification

- Focused hostile batch: 9/9 required GameTests passed.
- Final focused hostile batch: 9/9 required GameTests passed in 3.227s.
- Final aggregate live batch: 125/125 required GameTests passed in 47.41s; aggregate `check` completed in 1m14s.
