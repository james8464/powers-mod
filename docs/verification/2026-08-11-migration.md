...
- Action aliases loaded from `powers_actions` resolve retired IDs to current canonical IDs through a bounded acyclic graph. Artifact selections and favourites persist their migrated canonical menu keys; spell and crystal owners now persist stable action IDs alongside the legacy integer maps. Old saves without the new key maps decode with the existing deterministic index defaults, while future reloads migrate stable IDs without relying on page order.
