...
This matrix maps the requested finalisation work to committed implementation or repeatable verification evidence. “Complete” means the requested system and its automated evidence exist; subjective balance and appearance still receive the manual release playtest described in the README.

| ID | Requirement | Status | Evidence |
| --- | --- | --- | --- |
| R01 | Full production-code audit | Complete | `docs/quality/code-audit.md`; `scripts/audit_java_sources.py`; `validateCodeAudit` |
...
| R08 | Exhaustive power collision system | Complete | 97-action generated catalogue, all 4,753 pairs in `docs/interactions/interaction-matrix.csv`, drift tests |
...
| R18 | Light and Dark realm expansion | Complete | White Light sky renderer, six memory sites per realm, lore/rewards, mindscape biomes and resource validation |
...
| R21 | Named-mob remote viewing | Complete | Unique loaded-name resolver, 10,000-entity refusal bound, player consent, mob camera and vulnerable body |
...
| R24 | Vanilla-aligned ten-symbol energy HUD | Complete | `EnergyHudRenderer`, `HudLayout`, four symbol palettes, hunger alignment and `HudLayoutTest` |
...
| R32 | Shadow Sword apotheosis and custom powers | Complete | Seven original and eleven new rites; rank-10 cooldown bypass and 900-energy refill tests |
...
| R36 | Celestial Ruin containment spell | Complete | One-minute persistent 100-block beam, chunk tickets, restart-safe cursor, 120-block/2,000-damage bounded wave |
...
| R41 | Complete player documentation | Complete | README explains every innate power, spell, crystal, artifact, realm, rank, mob, command, configuration and safety boundary |
| R42 | Game tests and release validation harness | Complete | JUnit rule suites, Fabric `runGameTest`, resource/code/asset validators, dedicated client/server run tasks |
