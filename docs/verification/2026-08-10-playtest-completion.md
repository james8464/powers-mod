...
| Power behaviour | Lightning has no cooldown; Size Morphing is a selectable player power with distance-scaled upkeep; Time Slow and the five newly retired powers are absent; Time Freeze uses true server tick freezing. | Ability/rules tests and generated action catalogue |
...
- The previously recorded dedicated-server smoke initialized the older 28-power/82-action build. A fresh 23-power/77-action/3,003-interaction server boot is required before release and is not claimed by this interim checkpoint.
