...
- Damage uses radial falloff, deterministic nearest-first target order, a per-strike target cap, a per-body repeat interval, and a per-storm hit cap. Forcefields consume integrity through the ordinary power-damage bridge. Successful Insight strikes reveal power invisibility and briefly outline the body. Wardcraft diverts at most sixteen hostile projectiles without stealing ownership. Communion authors one bounded mirrored echo on every third regular strike. Might and Dominion add consent-safe shock pressure.
