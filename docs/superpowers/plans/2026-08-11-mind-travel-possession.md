...
- [x] Add failing tests for both-side lock, nested refusal, disconnect cleanup, 30-second ceiling, higher-rank refusal, amethyst refusal, mob support, attack/inventory forwarding, and no-power forwarding.
- [x] Implement server-authenticated movement/look/hotbar/attack forwarding with bounded deltas and target authority; freeze the target’s own input while possessed.
- [x] Reuse the core for Dreamwalking; run focused tests and live player/mob/test-actor GameTests.
- [x] Commit as `feat: implement authoritative vessel control`.
...
- [x] Add failing tests for avatar immunity, nonfatal proxy mirroring, fatal return-before-death, logout, missing dimension, duplicate cleanup, and vulnerable teleport-selection bodies.
- [x] Implement one cleanup owner and bounded retry/locked holding state with administrator diagnostics.
- [x] Run all body-proxy, realm-confinement, combat, and GameTests.
- [x] Commit as `fix: make mind sessions death-safe`.
...
- [x] Add failing tests for midpoint transfer, dual lightning, body lifecycle, unloaded chunks, nearest non-suffocating resolution, border/protection rejection, cancellation cleanup, unique-name ambiguity, and menu realm visibility.
- [x] Implement menu with only self coordinates and near unique name; rename every user-facing Time Shift string/ID migration to Teleport; hide Middleworld.
- [x] Route both ordinary and near-entity teleport through the storm and test distant destinations live.
- [x] Commit as `feat: unify safe teleport storms`.
...
- [x] Add failing tests for target beam, crouch group, player proxy, mob NoAI restore, crystal activation, fixed destination, return storm, invalid affinity, and every forbidden/allowed departure source.
- [x] Implement shared mind-session transport and strict `PLAYER_RETURN` versus `ADMIN_RECOVERY` policy.
- [x] Run realm policy, crystal, proxy, chunk loader, and live GameTests.
- [x] Commit as `fix: complete mindscape crystal travel`.
...
- [x] Add failing catalogue and round-trip persistence tests including restart, occupied origin fallback, and abandoned session cleanup.
- [x] Remove Portal Rift, implement persisted origin and shared storm, then run focused and live tests.
- [x] Commit as `fix: make indigo a reliable Middleworld passage`.
