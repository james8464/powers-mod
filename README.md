...
A darkness-tagged player carrying the Shadow Sword may press `G`, say `shadow, come to me`, or ask any `shadow, …` question to call Shadow explicitly. Merely holding the sword never spawns it. One authoritative, skin-matched player-model body exists on the server whether hidden or revealed; it never copies the owner's items or armour. Hidden presentation is owner-only, collisionless, invulnerable and ticket-free. Revealing changes that same body's tracking, collision and mortality without replacing it. Shadow follows at 5 Hz to a rear-side doglike position and uses a bounded eclipse teleport beyond 12 blocks. Killing a revealed body collapses its toggles, targets, tasks and manifestation, but preserves owner-keyed conversation, preferences and learned combat state; the sword may recall it after 100 ticks with 25% Darkness energy.

Shadow is private by default. `shadow, reveal yourself` makes both its manifestation and replies global; the body is naturally visible only to observers sharing its dimension. `shadow, hide yourself` makes the same body private again and returns replies to owner-only delivery. `shadow, leave me` dismisses it completely. Artifact/alignment loss, logout, owner death, three failed dimension transfers and server stop release the manifestation; astral projection, possession and mindscape travel do not create a second Shadow or silently discard it. All addressed messages are removed from ordinary signed chat and answered as clearly attributed server messages; unrelated chat remains untouched.

Shadow accepts typed tasks such as `follow`, `stay`, `guard`, `stop`, `attack <unique name>`, `defend me`, `fight close/ranged/automatically`, `use <power>`, `stop <power>`, `get <item>`, `conjure <item>`, `scout`, and diagnostic or lore questions. It resolves registry IDs and unique entity names deterministically, keeps only one foreground task, times work out safely and remembers at most 24 redacted conversation turns plus typed referents. Retrieval inspects at most 64 nearby dropped-item candidates within 32 blocks. Conjuration permits one plain stack of policy-approved ordinary items; administration items, spawn eggs, mythic artifacts, the Shadow Sword, Heavenly Partisan, third-party items without an opt-in tag and every crystal are forbidden. The sole item exception is one Dark Crystal, requiring all 1,850 energy and a visible, stationary, interruptible 60-second rite.

In combat Shadow has its own 1,850-point Darkness pool and can execute every one of the 23 max-Darkness innate powers plus Call the Hollowed, Blight Ground and Nightfall Dominion; it never uses crystal abilities. It is affected by amethyst, Pure Light, Darkness, safe zones, fields, sacrificial forcefields, spells, damage and global time state through the same participant/protection contracts as players. Every action has a named entity-safe executor—there is no pretend-success fallback—and active toggles are source-owned and removed on death, dismissal or sword loss.

The server gathers at most 64 target candidates, plans once per ten ticks and casts at most once per twenty ticks. Its utility planner selects close assault, skirmishing, ranged pressure, owner rescue or recovery, then approaches, orbits, retreats or interposes accordingly while avoiding allied firing lanes. A bounded contextual learner credits the preceding action for damage achieved versus harm suffered, adapts legal-action scores by at most ±25%, and uses at most 5% safe exploration. Learning is owner-local, contains no UUIDs, stores at most 64 contexts and 32 target archetypes, never authorizes an illegal action, and can be erased with `/powers shadow learning reset <player>`. No AI inference, navigation or learning runs on clients.
...
The launcher resolves Java 25 from `POWERS_JAVA_HOME`, a valid `JAVA_HOME`, Minecraft's bundled macOS runtime, Homebrew, or `PATH`. `check` includes 676 deterministic JUnit cases plus strict validation of JSON, duplicate keys, PNG headers/alpha/dimensions, Ogg/Vorbis streams, particle and sound references, models, translations, namespace safety, dimension biomes, release metadata, production-source reachability, exhaustive interaction/lifecycle-document drift, every production Java source, all non-item assets, and intentionally absent crystal recipes. `gametest` boots a real Fabric server and runs 56 registered live tests covering critical combat, artifacts, travel, bodies, creatures, realm forces, progression, testing tools, catastrophic magic, crystal travel, beam collisions, detached-avatar and physical-proxy death, vessel wrath, Shadow mortality/conjuration/combat, and the historical Cinderheart block-impact crash. `soak` exercises the actual spatial indexes, rotating queues, physical ray caps, Shadow planning/learning bounds, lifecycle cleanup and global budgets for deterministic 10/50/100-player workloads. The release JAR is written to `build/libs/`.
