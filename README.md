...
The [VFX-006 casting-pose evidence](docs/verification/evidence/2026-08-28-vfx-006/README.md) binds compact server-authoritative `INVOKE`, `PROJECT`, `CHANNEL`, and `RELEASE` poses for Shadow, both artifact guardians, both Heralds, and First Vessel. Current trackers receive bounded semantic state; late tracking retains authoritative age, and UUID/sequence/world-epoch checks clear replay, expiry, interruption, entity-ID reuse, and reconnect safely. POWERS-only model adapters preserve vanilla locomotion and held items without a global humanoid hook or animation dependency. The checksum-bound 54-frame real-client gallery covers all six styles in normal/reduced modes plus six lifecycle scenarios, and the literal aggregate passes 145 GameTests, 1,760 JUnit tests, and 191 Python tests.
