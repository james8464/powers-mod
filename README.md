...
| Fireball | One server-owned chargeable Cinderheart per caster; recast charges and crouch-use deliberately releases it without changing ownership. Punch catch/deflection remains finite, and impact creates bounded terrain/fire instead of vanilla explosion grief. Simulation and remaining world-time lifetime pause under every global freeze; lifecycle loss and local-freeze interruption still clean up. |
...
| Time Freeze | Owns Minecraft's true global tick freeze across loaded dimensions through a source-matched lease with no fixed deadline. Before activation it reports the exact per-second drain and safe whole seconds, with an advisory warning above 50 MSPT. Upkeep consumes at least 15% of full capacity each control-clock second; every external tick-state write supersedes POWERS without being undone. |
...
| Blue | **Chrono Stop:** toggles true global tick freeze, shows owner/deadline state plus a temporal-fracture edge, and auto-releases after exactly 1,200 control ticks without upkeep, even while world time is parked. **Dreamwalking:** controls one uniquely named consented player or mob for up to 30 seconds through the vulnerable body channel; right-click again performs an authenticated emergency return even though the controller is spectating the host. |
...
Crouch-right-click the Shadow Sword or Heavenly Partisan for a non-pausing eight-segment wheel. Hover and release crouch, click, or press `1`–`8` to select a favourite; release-to-cast is a separately persisted opt-in and defaults off. The centre opens a virtual scrolling icon library with Favourites, Recent, Innate, Crystals, and alignment-action tabs plus localized global search. Its fixed visible widget pool does not grow or rebuild while scrolling thousands of datapack actions. Entries show glyph, translated name, registry key, exact live cost/savings, cooldown progress, toggle, rank lock, and applicable size/gravity variant. Click a searched result and then slot `1`–`8` to request a bind in two interactions; the server validates it and refreshes the loadout before that action can execute. Loadouts and decode-bounded per-alignment recents persist, retired keys reconcile without duplicates, and newer revision refreshes retain the scroll position and any still-canonical selection.
...
Temporal leases distinguish innate, Crystal, and Shadow sources, with token/body-matched release and conservative crash-journal recovery that never thaws an ambiguous external freeze. Innate and Shadow Time Freeze have no fixed deadline; upkeep, reconciliation, and release presentation use control time. Channels, fields, Celestial Ruin, realm cycles, and physical-presence expiry use authoritative world time and pause under both owned and external vanilla freezes. The repaired implementation has independent READY review; final recapture and integration remain pending in the [INT-008 verification report](docs/verification/task-int-008-report.md) and its evidence closure ledger.
...
The accepted [VFX-011 exact visual-audit successor](docs/verification/evidence/2026-08-24-vfx-011/README.md) binds the exact implementation/JAR to the 970-asset audit, all 90 inspected pixel pages, a fresh 971-frame gallery with every raw PNG and original emitted metadata retained, 2,080 explicit digest-bound decisions, and passing literal aggregates. Contact sheets and non-opened raw captures remain explicitly limited rather than inferred PASS. The historical provisional bundle is preserved independently.

The accepted [VFX-009 Light Realm sky evidence](docs/verification/evidence/2026-08-25-vfx-009/README.md) binds full implementation/evidence commit `e78bf8f01fc79d6e05838c083685d348c214502c` and POWERS JAR SHA-256 `80147beb0793e37bc19d49133d11bde57b4c9c07dfff5b80c32bce25ce15d074` to Java 25 Fabric and pinned Sodium 0.9.1+mc26.2 nine-row normal/reduced/fallback galleries, raw PNG digests, and 18 full-resolution visual decisions. The deterministic upward camera makes the large warm-white/gold radial rune/halo visible in normal mode; reduced motion keeps two large static lower-contrast silhouettes; closing the enhancement leaves the pure-white `STATIC_WHITE` fallback with no enhanced silhouettes. The gallery covers distances 4/12/24, a resource reload, and a rain-command row. That command succeeded, but Light Realm has no skylight and the client observed `weather=clear`, so this is not a rendered-rain claim. The pinned Sodium JAR remains outside Git in the immutable compatibility cache; evidence retains its name, version, size, SHA-256, source URL, loaded-mod log, config, and options receipts. No third-party resource-pack live matrix was captured: the untextured built-in pipeline and source/architecture checks support resource-pack independence, but are not visual proof.

The [VFX-005 rank-ten silhouette evidence](docs/verification/evidence/2026-08-26-vfx-005/README.md) binds implementation commit `562f092a1393cb05485499fbbfc9b6782ae9b5cc` to all 23 unique normal and reduced-motion silhouettes, a fresh exact 56-frame real-client gallery, 253 pairwise monochrome distinctions, opaque-wall and near-body safety, lifecycle continuity, and the passing literal aggregate. The retained evidence is checksum-bound and privacy-sanitized. Its independent acceptance review returned `READY`; the closure head passed literal verification and VFX-005 is integrated into and pushed on `main`.

The [VFX-006 casting-pose evidence](docs/verification/evidence/2026-08-28-vfx-006/README.md) binds compact server-authoritative `INVOKE`, `PROJECT`, `CHANNEL`, and `RELEASE` poses for Shadow, both artifact guardians, both Heralds, and First Vessel. Current exact-dimension trackers receive bounded semantic state only after committed actions; late tracking retains authoritative age, while terminal packets and UUID/sequence/world-epoch checks clear replay, expiry, interruption, entity-ID reuse, and reconnect safely. POWERS-only model adapters preserve vanilla locomotion and held items without a global humanoid hook or animation dependency. The checksum-bound schema-2 real-client gallery covers 48 style/pose/motion frames plus seven lifecycle/compatibility scenarios, including actual delayed delivery and server-driven moving locomotion. Server intake is hard-bounded to 64 attempts per tick, with eight terminal slots reserved beyond the 56-attempt start lane so start-lane saturation cannot suppress up to eight clears. The literal Java 25 aggregate passes 153 GameTests, 1,773 JUnit tests, and 195 Python tests.

The accepted [VFX-007 layered-audio evidence](docs/verification/evidence/2026-08-28-vfx-007/README.md) binds implementation commit `1e04d2c2534120a234ebba2d04229a6f44a240d1` to 16 semantic cues and 51 deterministic near/mid/far plus reduced-tinnitus assets. A dedicated server and two real clients produced 77 privacy-safe audit rows covering all 48 open-distance combinations, every obstructed cue, capped bursts, ordinary/reduced Celestial playback, and reload/reconnect/dimension resets; all 16 localized subtitles have retained screenshots. Delivery, payload construction, admission, coalescing, obstruction, missing-resource fallback, and headroom are bounded without changing gameplay. Independent review returned `READY`; the literal Java 25 aggregate passes 155 GameTests, 1,810 JUnit tests, and 218 Python tests.

The [QA-001 release-envelope infrastructure](docs/verification/task-qa-001-report.md) is implemented but QA-001 remains open. A schema-checked catalogue owns literal no-shell gates; typed validators bind exact test counts, soak duration, real-client profiles, retained visual bytes, compatibility artifacts, and explicit limitations to one verified commit. The manual-only GitHub workflow attests the runtime JAR, sources JAR, JSON envelope, and Markdown envelope with `actions/attest@v4`; it cannot push, tag, or create a release. The committed final evidence index uses the exact `@HEAD` token, which is resolved only after proving clean single-branch `main` and equality with `origin/main`.

Final operators run the catalogue through `scripts/release_gate.py`, build the envelope with `scripts/release_envelope.py --mode final`, and dispatch `release-envelope.yml` with the full release SHA plus `config/release/qa-001-evidence.json`. After downloading the bundle, run all four generated `gh attestation verify <subject> --repo james8464/powers-mod` commands. A tag is permitted only after every subject verifies; infrastructure presence or workflow-artifact upload is not acceptance.

Source quality is executable rather than hash-inferred. The audit parses comments separately from literals and uses the JDK syntax tree for annotated public-type declarations, callable integration APIs, and externally visible behavioural owners. It rejects unfinished markers, mechanical narration, unsupported certainty, undocumented contracts, wildcard/debug shortcuts, oversized files, and near-limit source units with multiple real public owners. Package-private/private helpers remain internal, interface member types retain Java's implicit-public semantics, and only exact `Override`/`java.lang.Override` annotations inherit a callable contract. The exact [Java source manifest](docs/quality/code-audit.md) identifies the reviewed bytes and public type contract; focused behavior tests remain the proof of semantics.
