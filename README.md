...
Source quality is executable rather than hash-inferred. The audit parses comments separately from literals, rejects unfinished markers, short/mechanical narration, unsupported certainty, undocumented callable integration APIs, wildcard/debug shortcuts, oversized files, and near-limit source units with multiple top-level owners. The exact [Java source manifest](docs/quality/code-audit.md) identifies the reviewed bytes and public type contract; focused behavior tests remain the proof of semantics.
