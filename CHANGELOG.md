...
- Implemented the open VFX-011 audit pipeline: all 970 assets and 362 PNGs have resolved graph/frame/mip evidence and 90 deterministic pages, while a clean two-client campaign proves the locator and Darkness root. Evidence review found the historical 971 gallery raw files/original metadata were not retained; its decisions are now pending, and the repaired agent/packager require actual emitted settings, raw-byte rehashing, and content-addressed retention. Fresh post-soak capture/review and the literal aggregate remain required; VFX-009 stays separate and open.
