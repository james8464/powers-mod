...
- Implemented the provisional VFX-011 exact visual-audit pipeline: all 970 assets and 362 PNGs have resolved graph/frame/mip evidence, 90 deterministic light/dark/checker pages, explicit digest-bound review decisions, and a 971-frame production client gallery spanning item transforms, spawn eggs, entities, screens, HUD states, boss bars, and real gameplay cameras. A clean two-client campaign proves the entity locator and Darkness advancement root. The row remains open until the protected QA-006 soak ends and one final literal aggregate gate can run without host-load interference; VFX-009 remains separate and open.
