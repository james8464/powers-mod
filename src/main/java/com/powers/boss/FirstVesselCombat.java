...
public final class FirstVesselCombat {
	...
	public static boolean cast(ServerLevel level, FirstVessel boss, LivingEntity target,
			FirstVesselPowerAction action, FirstVesselPhase phase) {
		if (!target.isAlive()) return false;
		boolean committed = switch (action.powerId()) {
		...
		if (committed && phase == FirstVesselPhase.LAST_COVENANT) {
	...
	public static void worldSuture(ServerLevel level, FirstVessel boss) {
		...
		CastingPoseService.start(boss, com.powers.animation.CastingPose.RELEASE,
				com.powers.animation.CastingStyle.FIRST_VESSEL, CastingHand.BOTH, 20);
	}
	...
	private static boolean lightning(ServerLevel level, FirstVessel boss, LivingEntity target) {
		if (!harm(level, boss, target, 70.0F)) return false;
	...
	private static boolean projectile(ServerLevel level, FirstVessel boss, LivingEntity target,
			float damage, int color) {
		if (!harm(level, boss, target, damage)) return false;
	...
	private static boolean beam(ServerLevel level, FirstVessel boss, LivingEntity target,
			float damage, int color) {
		if (!harm(level, boss, target, damage)) return false;
		PowerFx.beam(level, boss.getEyePosition(), target.getEyePosition(),
	...
	private static boolean area(ServerLevel level, FirstVessel boss, Vec3 center,
			double radius, float damage, boolean vertical) {
		boolean committed = false;
		for (LivingEntity target : candidates(level, boss, center, radius)) {
			if (!harm(level, boss, target, damage)) continue;
			committed = true;
			if (!mayControl(level, target)) continue;
	...
	private static boolean thunderclap(ServerLevel level, FirstVessel boss) {
		if (!area(level, boss, boss.position(), 12.0, 58.0F, false)) return false;
		PowerFx.sound(level, boss.position(), SoundEvents.WARDEN_SONIC_BOOM, 2.0F, 1.4F);
		return true;
	}

	private static boolean throwTarget(ServerLevel level, FirstVessel boss, LivingEntity target) {
		if (!mayControl(level, target) || !harm(level, boss, target, 46.0F)) return false;
	...
	private static boolean crush(ServerLevel level, FirstVessel boss, LivingEntity target) {
		if (!mayControl(level, target) || !harm(level, boss, target, 68.0F)) return false;
	...
	private static boolean freeze(ServerLevel level, FirstVessel boss, LivingEntity target) {
		boolean committed = false;
	...
	private static boolean drain(ServerLevel level, FirstVessel boss, LivingEntity target) {
		if (!harm(level, boss, target, 56.0F)) return false;
		recover(level, boss, 160.0F);
		PowerFx.beam(level, target.getEyePosition(), boss.getEyePosition(),
				com.powers.PowersParticles.ECLIPSE, 28);
		return true;
	}

	private static boolean iceLance(ServerLevel level, FirstVessel boss, LivingEntity target) {
		if (!harm(level, boss, target, 60.0F)) return false;
		if (mayControl(level, target)) {
	...
	private static boolean recover(ServerLevel level, FirstVessel boss, float amount) {
		float before = boss.getHealth();
		boss.heal(amount);
		if (boss.getHealth() <= before) return false;
	...
	private static boolean ward(ServerLevel level, FirstVessel boss, float scale, int absorptionTicks) {
	...
	private static boolean veil(ServerLevel level, FirstVessel boss) {
		if (!boss.addEffect(PowerStatusEffects.hidden(MobEffects.INVISIBILITY,
				100, 0, false, true))) return false;
		PowerFx.burst(level, boss.position(), com.powers.PowersParticles.ECLIPSE, 18, 0.7, 0.06);
		return true;
	}

	private static boolean rush(ServerLevel level, FirstVessel boss, LivingEntity target) {
		Vec3 direction = target.position().subtract(boss.position());
		if (direction.lengthSqr() <= 1.0E-6) return false;
		boss.setDeltaMovement(direction.normalize().scale(2.2).add(0, 0.35, 0));
		PowerFx.beam(level, boss.position(), target.position(), com.powers.PowersParticles.RIBBON, 18);
		return true;
	}

	private static boolean step(ServerLevel level, FirstVessel boss, LivingEntity target, double radius) {
	...
	private static boolean fallback(ServerLevel level, FirstVessel boss, LivingEntity target, Kind kind) {
		if (kind == Kind.MOBILITY) return rush(level, boss, target);
		if (kind == Kind.RECOVERY || kind == Kind.DEFENSE) return recover(level, boss, 120.0F);
		return projectile(level, boss, target, 48.0F, 0x6B2A79);
	}
