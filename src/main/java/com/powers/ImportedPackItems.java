...
public final class ImportedPackItems {
	...
	private static final String[] TEXTURES = """
		...
		""".split(",");
	private static final List<String> TEXTURE_IDS = java.util.Arrays.stream(TEXTURES)
			.map(String::trim).filter(texture -> !texture.isEmpty()).toList();
	...
	/** Stable source texture keys used by loot and exhaustive item-purpose audits. */
	public static List<String> textureIds() {
		return TEXTURE_IDS;
	}

	public static List<String> importedIds() {
		return TEXTURE_IDS.stream().map(texture ->
				"imported_" + texture.replace('.', '_')).toList();
	}

	public static void initialize() {
		for (String texture : TEXTURE_IDS) {
