...
class Vfx007AudioCaptureTest(unittest.TestCase):
    ...
    def test_primary_scenario_covers_open_wall_burst_comfort_reload_dimension_and_subtitles(self):
        ...
        self.assertEqual(1, sum(line.endswith("\tcommand\tfill -2 198 1 2 204 170 air")
                                for line in lines))
        post_wall_look = next(index for index, line in enumerate(lines)
                              if line.endswith("\tlook\t90 0"))
        burst = next(index for index, line in enumerate(lines)
                     if line.endswith("\taudio_emit\tinteraction_clash 1 open"))
        self.assertLess(post_wall_look, burst)
